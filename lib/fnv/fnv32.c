 ...
 */
static int test_fnv32(enum fnv_type hash_type, Fnv32_t init_hval, Fnv32_t mask, int v_flag, int code) {
                    ...
                    case FNV0_32:
                        if ((hval & mask) != (fnv0_32_vector[tstnum - 1].fnv0_32 & mask)) {
                            if (v_flag) {
                                ...
                                fprintf(stderr, "%s: expected 0x%08lx != generated: 0x%08lx\n", program, (hval & mask), (fnv0_32_vector[tstnum - 1].fnv0_32 & mask));
                            }
                            return tstnum;
                        }
                        break;
                    case FNV1_32:
                        if ((hval & mask) != (fnv1_32_vector[tstnum - 1].fnv1_32 & mask)) {
                            if (v_flag) {
                                ...
                                fprintf(stderr, "%s: test # 1 is 1st test\n", program);
                                fprintf(stderr, "%s: expected 0x%08lx != generated: 0x%08lx\n", program, (hval & mask), (fnv1_32_vector[tstnum - 1].fnv1_32 & mask));
                            }
                            return tstnum;
                        }
                        break;
                    case FNV1a_32:
                        if ((hval & mask) != (fnv1a_32_vector[tstnum - 1].fnv1a_32 & mask)) {
                            if (v_flag) {
                                ...
                                fprintf(stderr, "%s: test # 1 is 1st test\n", program);
                                fprintf(stderr, "%s: expected 0x%08lx != generated: 0x%08lx\n", program, (hval & mask), (fnv1a_32_vector[tstnum - 1].fnv1a_32 & mask));
                            }
                            return tstnum;
                        }
                        break;
