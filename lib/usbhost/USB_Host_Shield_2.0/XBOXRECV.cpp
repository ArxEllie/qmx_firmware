...
void XBOXRECV::readReport(uint8_t controller) {
        ...
        if (Xbox360Connected[controller]) {
            ...
            switch (readBuf[1]) {
                ...
                case 0x40:
                    ...
                    break;
                case 0xC0:
                    str = PSTR(" as controller+headset\r\n");
                    break;
            }
    ...
    if (!Xbox360Connected[controller]) Xbox360Connected[controller] |= 0x80;
    ...
    if (ButtonState[controller] != OldButtonState[controller]) {
        ...
        if ((uint8_t)(OldButtonState[controller] >> 8) == 0 && (uint8_t)(ButtonState[controller] >> 8) != 0) L2Clicked[controller] = true;
...
void XBOXRECV::checkStatus() {
    ...
    writeBuf[3] = 0x40;
    for (uint8_t i = 0; i < 4; i++) {
        if (Xbox360Connected[i]) XboxCommand(i, writeBuf, 4);
    }
}
