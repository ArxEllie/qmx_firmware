...
void SPP::disconnect() {
    ...
    if (RFCOMMConnected) pBtd->l2cap_disconnection_request(hci_handle, ++identifier, rfcomm_scid, rfcomm_dcid);
    if (RFCOMMConnected && SDPConnected) delay(1); // Add delay between commands
    if (SDPConnected) pBtd->l2cap_disconnection_request(hci_handle, ++identifier, sdp_scid, sdp_dcid);
...
void SPP::ACLData(uint8_t *l2capinbuf) {
    ...
    if (checkHciHandle(l2capinbuf, hci_handle)) {                // acl_handle_ok
        if ((l2capinbuf[6] | (l2capinbuf[7] << 8)) == 0x0001U) { // l2cap_control - Channel ID for ACL-U
            ...
            if (rfcommChannel >> 3 != 0x00) rfcommChannelConnection = rfcommChannel;
...
void SPP::onInit() {
    ...
    sppIndex           = 0;
    if (pFuncOnInit) pFuncOnInit(); // Call the user function
};
...
bool SPP::checkFcs(uint8_t *data, uint8_t fcs) {
    uint8_t temp = crc(data);
    if ((data[1] & 0xEF) != RFCOMM_UIH) temp = pgm_read_byte(&rfcomm_crc_table[temp ^ data[2]]); // FCS on 3 bytes
...
void SPP::write(const uint8_t *data, size_t size) {
    ...
    for (uint8_t i = 0; i < size; i++) {
        if (sppIndex >= sizeof(sppOutputBuffer) / sizeof(sppOutputBuffer[0])) send(); // Send the current data in the buffer
