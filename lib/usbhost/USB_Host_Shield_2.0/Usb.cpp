...
EpInfo *USB::getEpInfoEntry(uint8_t addr, uint8_t ep) {
    ...
    if (!p || !p->epinfo) return NULL;

    EpInfo *pep = p->epinfo;

    for (uint8_t i = 0; i < p->epcount; i++) {
        if ((pep)->epAddr == ep) return pep;
...
uint8_t USB::setEpInfoEntry(uint8_t addr, uint8_t epcount, EpInfo *eprecord_ptr) {
    if (!eprecord_ptr) return USB_ERROR_INVALID_ARGUMENT;
...
uint8_t USB::SetAddress(uint8_t addr, uint8_t ep, EpInfo **ppep, uint16_t *nak_limit) {
    ...
    *ppep = getEpInfoEntry(addr, ep);
    ...
    if (!*ppep) return USB_ERROR_EP_NOT_FOUND_IN_TBL;
...
uint8_t USB::OutTransfer(EpInfo *pep, uint16_t nak_limit, uint16_t nbytes, uint8_t *data) {
    ...
    if (maxpktsize < 1 || maxpktsize > 64) return USB_ERROR_INVALID_MAX_PKT_SIZE;
    ...
    while (bytes_left) {
        ...
        while (rcode && ((long)(millis() - timeout) < 0L)) {
            switch (rcode) {
                case hrNAK:
                    nak_count++;
                    if (nak_limit && (nak_count == nak_limit)) goto breakout;
                    // return ( rcode);
                    break;
                case hrTIMEOUT:
                    retry_count++;
                    if (retry_count == USB_RETRY_LIMIT) goto breakout;
                    // return ( rcode);
                    break;
                case hrTOGERR:
                    // yes, we flip it wrong here so that next time it is actually correct!
...
uint8_t USB::dispatchPkt(uint8_t token, uint8_t ep, uint16_t nak_limit) {
    ...
    while ((long)(millis() - timeout) < 0L) {
        ...
        switch (rcode) {
            case hrNAK:
                nak_count++;
                if (nak_limit && (nak_count == nak_limit)) return (rcode);
                break;
            case hrTIMEOUT:
                retry_count++;
                if (retry_count == USB_RETRY_LIMIT) return (rcode);
                break;
            default:
                return (rcode);
        } // switch( rcode
...
void USB::Task(void) // USB state machine
{
    ...
    switch (tmpdata) {
        ...
        case SE0: // disconnected
            if ((usb_task_state & USB_STATE_MASK) != USB_STATE_DETACHED) usb_task_state = USB_DETACHED_SUBSTATE_INITIALIZE;
            lowspeed = false;
            break;
        case LSHOST:
    ...
    switch (usb_task_state) {
        case USB_DETACHED_SUBSTATE_INITIALIZE:
            ...
            for (uint8_t i = 0; i < USB_NUMDEVICES; i++)
                if (devConfig[i]) rcode = devConfig[i]->Release();
...
uint8_t USB::Configuring(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    for (devConfigIndex = 0; devConfigIndex < USB_NUMDEVICES; devConfigIndex++) {
        ...
        if (devConfig[devConfigIndex]->DEVSUBCLASSOK(subklass) && (devConfig[devConfigIndex]->VIDPIDOK(vid, pid) || devConfig[devConfigIndex]->DEVCLASSOK(klass))) {
            ...
            if (rcode != USB_DEV_CONFIG_ERROR_DEVICE_NOT_SUPPORTED) break;
        }
    }
...
uint8_t USB::ReleaseDevice(uint8_t addr) {
    ...
    for (uint8_t i = 0; i < USB_NUMDEVICES; i++) {
        ...
        if (devConfig[i]->GetAddress() == addr) return devConfig[i]->Release();
    }
    return 0;
}
