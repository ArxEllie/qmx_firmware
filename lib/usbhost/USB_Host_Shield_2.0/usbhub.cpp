...
uint8_t USBHub::Init(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    if (!rcode) len = (buf[0] > 32) ? 32 : buf[0];
    ...
    if (udd->bDeviceClass != 0x09) return USB_DEV_CONFIG_ERROR_DEVICE_NOT_SUPPORTED;
...
uint8_t USBHub::Release() {
    ...
    if (bAddress == 0x41) pUsb->SetHubPreMask();
...
uint8_t USBHub::CheckHubStatus() {
    ...
    for (uint8_t port = 1; port <= bNbrPorts; port++) {
        ...
        if ((evt.bmStatus & bmHUB_PORT_STATE_CHECK_DISABLED) != bmHUB_PORT_STATE_DISABLED) continue;
