...
class ReportDescParserBase : public USBReadParser {
   ...
   public:
    ReportDescParserBase() : itemParseState(0), itemSize(0), itemPrefix(0), rptSize(0), rptCount(0), pfUsage(NULL) {
...
class ReportDescParser2 : public ReportDescParserBase {
   ...
   public:
    ReportDescParser2(uint16_t len, uint8_t *pbuf) : ReportDescParserBase(), rptId(0), useMin(0), useMax(0), fieldCount(0), pBuf(pbuf), bLen(len) {};
};
