...
class KeyboardReportParser : public HIDReportParser {
   ...
   protected:
    virtual uint8_t HandleLockingKeys(HID *hid, uint8_t key) {
        ...
        if (old_keys != kbdLockingKeys.bLeds && hid) return (hid->SetReport(0, 0 /*hid->GetIface()*/, 2, 0, 1, &kbdLockingKeys.bLeds));
...
template <const uint8_t BOOT_PROTOCOL>
HIDBoot<BOOT_PROTOCOL>::HIDBoot(USB *p) : HID(p), qNextPollTime(0), bPollEnable(false) {
    Initialize();
...
uint8_t HIDBoot<BOOT_PROTOCOL>::Init(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    if ((BOOT_PROTOCOL & (HID_PROTOCOL_KEYBOARD | HID_PROTOCOL_MOUSE)) == (HID_PROTOCOL_KEYBOARD | HID_PROTOCOL_MOUSE)) {
        ...
        ConfigDescParser<USB_CLASS_HID, HID_BOOT_INTF_SUBCLASS, HID_PROTOCOL_KEYBOARD | HID_PROTOCOL_MOUSE, CP_MASK_COMPARE_ALL> confDescrParser(this);
        confDescrParser.SetOR(); // Use the OR variant.
        for (uint8_t i = 0; i < num_of_conf; i++) {
            pUsb->getConfDescr(bAddress, 0, i, &confDescrParser);
            if (bNumEP == (uint8_t)(totalEndpoints(BOOT_PROTOCOL))) break;
        }
    } else {
        // GCC will optimize unused stuff away.
        if (BOOT_PROTOCOL & HID_PROTOCOL_KEYBOARD) {
            ...
            for (uint8_t i = 0; i < num_of_conf; i++) {
                ConfigDescParser<USB_CLASS_HID, HID_BOOT_INTF_SUBCLASS, HID_PROTOCOL_KEYBOARD, CP_MASK_COMPARE_ALL> confDescrParserA(this);
        ...
        if (BOOT_PROTOCOL & HID_PROTOCOL_MOUSE) {
            ...
            for (uint8_t i = 0; i < num_of_conf; i++) {
                ConfigDescParser<USB_CLASS_HID, HID_BOOT_INTF_SUBCLASS, HID_PROTOCOL_MOUSE, CP_MASK_COMPARE_ALL> confDescrParserB(this);
...
void HIDBoot<BOOT_PROTOCOL>::EndpointXtract(uint8_t conf, uint8_t iface, uint8_t alt, uint8_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
    ...
    if (bNumEP == totalEndpoints(BOOT_PROTOCOL)) return;
...
uint8_t HIDBoot<BOOT_PROTOCOL>::Poll() {
    ...
    if (bPollEnable && ((long)(millis() - qNextPollTime) >= 0L)) {
        ...
        for (int i = 0; i < epMUL(BOOT_PROTOCOL); i++) {
            ...
            if (!rcode && read > 2) {
                if (pRptParser[i]) pRptParser[i]->Parse((HID *)this, 0, (uint8_t)read, buf);
#    ifdef DEBUG_USB_HOST
