...
void PS4Parser::Parse(uint8_t len, uint8_t *buf) {
    if (len > 1 && buf) {
        ...
        if (ps4Data.btn.val != oldButtonState.val) {                      // Check if anything has changed
            ...
            uint8_t newDpad = 0;
            if (checkDpad(UP)) newDpad |= 1 << UP;
            if (checkDpad(RIGHT)) newDpad |= 1 << RIGHT;
            if (checkDpad(DOWN)) newDpad |= 1 << DOWN;
            if (checkDpad(LEFT)) newDpad |= 1 << LEFT;
            if (newDpad != oldDpad) {
    ...
    if (ps4Output.reportChanged) sendOutputReport(&ps4Output); // Send output report
}
