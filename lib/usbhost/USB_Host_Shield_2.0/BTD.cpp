...
#endif
    if (rcode != hrJERR) rcode = USB_ERROR_FailGetDevDescr;
    Release();
    return rcode;
};
...
uint8_t BTD::Init(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    if (VID == PS3_VID && (PID == PS3_PID || PID == PS3NAVIGATION_PID || PID == PS3MOVE_PID)) {
        ...
        if (bNumEP < BTD_MAX_ENDPOINTS) goto FailUnknownDevice;
...
void BTD::Initialize() {
    ...
    for (i = 0; i < BTD_NUM_SERVICES; i++) {
        if (btService[i]) btService[i]->Reset(); // Reset all Bluetooth services
    }
...
#ifdef DEBUG_USB_HOST
                            if (classOfDevice[0] & 0x80) Notify(PSTR("\r\nMouse found"), 0x80);
                            if (classOfDevice[0] & 0x40) Notify(PSTR("\r\nKeyboard found"), 0x80);
                            if (classOfDevice[0] & 0x08) Notify(PSTR("\r\nGamepad found"), 0x80);
#endif
    ...
    switch (hci_state) {
        ...
        case HCI_RESET_STATE:
            ...
            if (hci_check_flag(HCI_FLAG_CMD_COMPLETE)) {
                ...
                if (hci_num_reset_loops > 2000) hci_num_reset_loops = 2000;
#ifdef DEBUG_USB_HOST
...
#endif
                if (incomingPS4) connectToHIDDevice = true; // We should always connect to the PS4 controller
...
void BTD::ACL_event_task() {
    ...
    if (!rcode) {         // Check for errors
        if (length > 0) { // Check if any data was read
            for (uint8_t i = 0; i < BTD_NUM_SERVICES; i++) {
                if (btService[i]) btService[i]->ACLData(l2capinbuf);
            }
        }
    }
#ifdef EXTRADEBUG
