...
FTDI::FTDI(USB *p, FTDIAsyncOper *pasync) : pAsync(pasync), pUsb(p), bAddress(0), bNumEP(1), wFTDIType(0) {
...
uint8_t FTDI::Init(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    if (udd->idVendor != FTDI_VID || udd->idProduct != FTDI_PID) return USB_DEV_CONFIG_ERROR_DEVICE_NOT_SUPPORTED;
