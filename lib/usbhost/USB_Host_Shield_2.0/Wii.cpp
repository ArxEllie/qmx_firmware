...
#ifdef DEBUG_USB_HOST
                                if (!unknownExtensionConnected) Notify(PSTR("\r\nExtension connected"), 0x80);
#endif
...
#ifdef DEBUG_USB_HOST
                        if (l2capinbuf[12] & 0x01) Notify(PSTR("\r\nWARNING: Battery is nearly empty"), 0x80);
#endif
...
void WII::Run() {
    if (l2cap_state == L2CAP_INTERRUPT_DISCONNECT && ((long)(millis() - timer) >= 0L)) L2CAP_task(); // Call the rest of the disconnection routine after we have waited long enough
    ...
    switch (l2cap_state) {
...
#endif
            stateCounter++;
            if (stateCounter % 200 == 0) checkMotionPresent(); // Check if there is a motion plus connected
        ...
        case WII_CHECK_EXTENSION_STATE: // This is used to check if there is anything plugged in to the extension port
            ...
            if (stateCounter == 1) statusRequest(); // See if a new device has connected
            if (stateCounter == 100) {
        ...
        case TURN_ON_LED:
            if (wii_check_flag(WII_FLAG_NUNCHUCK_CONNECTED)) nunchuckConnected = true;
        ...
        case L2CAP_DONE:
            if (unknownExtensionConnected) {
                ...
                if (stateCounter == 50)
                    ...
                    if (!motionPlusConnected) stateCounter = 449;
                } else if (stateCounter == 300) {
                    if (motionPlusConnected) {
#ifdef DEBUG_USB_HOST
...
void WII::setLedStatus() {
    ...
    if (wiimoteConnected) HIDBuffer[2] |= 0x10;    // If it's connected LED1 will light up
    if (motionPlusConnected) HIDBuffer[2] |= 0x20; // If it's connected LED2 will light up
    if (nunchuckConnected) HIDBuffer[2] |= 0x40;   // If it's connected LED3 will light up
