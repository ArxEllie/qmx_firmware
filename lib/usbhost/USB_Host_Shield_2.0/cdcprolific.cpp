...
PL2303::PL2303(USB *p, CDCAsyncOper *pasync) : ACM(p, pasync), wPLType(0) {}
...
uint8_t PL2303::Init(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    if (udd->idVendor != PL_VID && CHECK_PID(udd->idProduct)) return USB_DEV_CONFIG_ERROR_DEVICE_NOT_SUPPORTED;
