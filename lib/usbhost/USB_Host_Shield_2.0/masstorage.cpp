...
uint32_t BulkOnly::GetCapacity(uint8_t lun) {
    if (LUNOk[lun]) return CurrentCapacity[lun];
    return 0LU;
}
...
uint16_t BulkOnly::GetSectorSize(uint8_t lun) {
    if (LUNOk[lun]) return CurrentSectorSize[lun];
    return 0U;
}
...
uint8_t BulkOnly::Init(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    for (uint8_t i = 0; i < num_of_conf; i++) {
        ConfigDescParser<USB_CLASS_MASS_STORAGE, MASS_SUBCLASS_SCSI, MASS_PROTO_BBB, CP_MASK_COMPARE_CLASS | CP_MASK_COMPARE_SUBCLASS | CP_MASK_COMPARE_PROTOCOL> BulkOnlyParser(this);
...
void BulkOnly::EndpointXtract(uint8_t conf, uint8_t iface, uint8_t alt, uint8_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
    ...
    else if ((pep->bmAttributes & 0x02) == 2)
        index = ((pep->bEndpointAddress & 0x80) == 0x80) ? epDataInIndex : epDataOutIndex;
    else
        return;
...
bool BulkOnly::CheckLUN(uint8_t lun) {
    ...
    if (CurrentCapacity[lun] == /*0xffffffffLU */ 0x01LU || CurrentCapacity[lun] == 0x00LU) {
        ...
        if (CurrentCapacity[lun]) ErrorMessage<uint8_t>(PSTR(">>>>>>>>>>>>>>>>BUGGY FIRMWARE. CAPACITY FAIL ON LUN"), lun);
        return false;
    }
    delay(20);
    Page3F(lun);
...
uint8_t BulkOnly::TestUnitReady(uint8_t lun) {
    // SetCurLUN(lun);
    if (!bAddress) return MASS_ERR_UNIT_NOT_READY;
 ...
 */
void BulkOnly::Reset() {
    while (pUsb->ctrlReq(bAddress, 0, bmREQ_MASSOUT, MASS_REQ_BOMSR, 0, 0, bIface, 0, 0, NULL, NULL) == 0x01)
        delay(6);
}
...
uint8_t BulkOnly::HandleUsbError(uint8_t error, uint8_t index) {
    ...
    while (error && count) {
        ...
        switch (error) {
            ...
            case hrSTALL:
                if (index == 0) return MASS_ERR_STALL;
                ClearEpHalt(index);
                if (index != epDataInIndex) return MASS_ERR_WRITE_STALL;
                return MASS_ERR_STALL;

            case hrNAK:
                if (index == 0) return MASS_ERR_UNIT_BUSY;
                return MASS_ERR_UNIT_BUSY;
...
uint8_t BulkOnly::Transaction(CommandBlockWrapper *pcbw, uint16_t buf_size, void *buf
    ...
    while ((usberr = pUsb->outTransfer(bAddress, epInfo[epDataOutIndex].epAddr, sizeof(CommandBlockWrapper), (uint8_t *)pcbw)) == hrBUSY)
        delay(1);
    ...
    if (ret) {
        ...
        if (bytes) {
            if (!write) {
                ...
                if (callback) {
                    uint8_t rbuf[bytes];
                    while ((usberr = pUsb->inTransfer(bAddress, epInfo[epDataInIndex].epAddr, &bytes, rbuf)) == hrBUSY)
                        delay(1);
                    if (usberr == hrSUCCESS) ((USBReadParser *)buf)->Parse(bytes, rbuf, 0);
                } else {
#endif
                    while ((usberr = pUsb->inTransfer(bAddress, epInfo[epDataInIndex].epAddr, &bytes, (uint8_t *)buf)) == hrBUSY)
                        delay(1);
#if MS_WANT_PARSER
                }
#endif
                ...
                while ((usberr = pUsb->outTransfer(bAddress, epInfo[epDataOutIndex].epAddr, bytes, (uint8_t *)buf)) == hrBUSY)
                    delay(1);
                ret = HandleUsbError(usberr, epDataOutIndex);
            }
            if (ret) {
...

    {
        ...
        int tries = 2;
        while (tries--) {
            while ((usberr = pUsb->inTransfer(bAddress, epInfo[epDataInIndex].epAddr, &bytes, (uint8_t *)&csw)) == hrBUSY)
                delay(1);
            if (!usberr) break;
            ClearEpHalt(epDataInIndex);
...
uint8_t BulkOnly::Read(uint8_t lun, uint32_t addr, uint16_t bsize, uint8_t blocks, USBReadParser *prs) {
    ...
    cbw.bmCBWFlags = MASS_CMD_DIR_IN, cbw.bmCBWLUN = lun;
