...
HIDUniversal::HIDUniversal(USB *p) : HID(p), qNextPollTime(0), pollInterval(0), bPollEnable(false), bHasReportId(false) {
    Initialize();
...
void HIDUniversal::EndpointXtract(uint8_t conf, uint8_t iface, uint8_t alt, uint8_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
    ...
    if (bNumEP > 1 && conf != bConfNum) return;
...
uint8_t HIDUniversal::Poll() {
    ...
    if ((long)(millis() - qNextPollTime) >= 0L) {
        ...
        for (uint8_t i = 0; i < bNumIface; i++) {
            ...
            if (rcode) {
                if (rcode != hrNAK) USBTRACE3("(hiduniversal.h) Poll:", rcode, 0x81);
                return rcode;
            }
