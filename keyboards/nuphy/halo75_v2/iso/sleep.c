...
void Sleep_Handle(void) {
    ...
    if (f_goto_sleep) {
        ...
        if(f_dev_sleep_enable) {
            ...
            // power off led
            gpio_write_pin(DC_BOOST_PIN, 0);
            gpio_write_pin(RGB_DRIVER_SDB1, 0);
            gpio_write_pin(RGB_DRIVER_SDB2, 0);
        }
    ...
    if (f_wakeup_prepare && (no_act_time < 10)) {
        ...
        gpio_write_pin(DC_BOOST_PIN, 1);
        gpio_write_pin(RGB_DRIVER_SDB1, 1);
        gpio_write_pin(RGB_DRIVER_SDB2, 1);
